...
/**
 * Troca de telas da aplicacao.
 *
 * <p>Equivalente desktop do roteador de uma SPA: como tudo roda num processo
 * so, "navegar" e apenas trocar a raiz da Scene, sem requisicao nenhuma.</p>
 */
public final class Navegador {
    ...
    public static void iniciar(Stage stage) {
        Font.loadFont(Navegador.class.getResourceAsStream("/fonts/Montserrat-Regular.ttf"), 14);
        Font.loadFont(Navegador.class.getResourceAsStream("/fonts/Montserrat-Bold.ttf"), 14);
        palco = stage;
    ...
    public static void irPara(Tela tela) {
        try {
            ...
            if (recurso == null) {
                Alertas.aviso("Operação indisponível",
            ...
            if (cena == null) {
                cena = new Scene(raiz, 960, 540);
                cena.getStylesheets().add(
                        Objects.requireNonNull(
                                Navegador.class.getResource("/css/sgb.css")).toExternalForm());
                palco.setScene(cena);
            } else {
                cena.setRoot(raiz);
            }
            ...
            throw new IllegalStateException("Falha ao carregar a tela " + tela, e);
        }
    }
